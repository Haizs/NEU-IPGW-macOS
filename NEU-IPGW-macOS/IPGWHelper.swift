...
class IPGWHelper: NSObject, URLSessionTaskDelegate {
    ...
    private func fetchCASTGC(onSuccess: @escaping() -> Void,
                             onFailure: @escaping(String) -> Void) {
        self.request(urlString: "https://pass.neu.edu.cn/tpass/login",
                     dataString: nil,
    ...
    func refreshCAS(retriedTimes: Int,
        ...
        self.request(urlString: url,
                     ...
                     onSuccess: { data, _ in
                         ...
                         self.calculateData(data: data, onSuccess: { data in
                                                onSuccess(data)
                                            }, onFailure: { error in
                                                onFailure(error)
                                            })
    ...
    private func calculateData(data: String,
        ...
        guard let username = regexMatches(for: #"(?<=登录帐号：<span id="user_name" style="float:right;color: #894324;">)[^<]*"#, in: data).first,
            let userip = regexMatches(for: #"(?<=当前IP：<span id="user_name" style="float:right;color: #894324;">)[^<]*"#, in: data).first,
            var totalBalance = regexMatches(for: #"(?<=已用流量：<span id="balance_in" style="float:right; color: #894324;">)[^<]*"#, in: data).first,
            var totalTime = regexMatches(for: #"(?<=已用时长：<span id="balance_out" style="float:right; color: #894324;">)[^<]*"#, in: data).first else {
                onFailure("数据加载失败")
                return
        }
    ...
    private func sumBalance(_ suma: String, _ sumb: String) -> String {
        ...
        if balance >= 1000 * 1000 * 1000 {
            ...
            return String(format: "%.0f,%.2fM", gPart, mPart)
